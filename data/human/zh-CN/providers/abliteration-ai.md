# abliteration.ai

- provider_id: `abliteration-ai`
- 渠道: 推理托管
- API 地址: `https://api.abliteration.ai/v1`
- 官网: https://docs.abliteration.ai/models
- 定价页: https://docs.abliteration.ai/models
- 币种: USD
- 数据更新时间: 2026-08-22T09:32:50Z
- 核实时间: 2026-08-22T09:32:50Z

共 **2** 个模型。

| 模型 | 状态 | 类别 | 上下文 | 输入 $/MTok | 输出 $/MTok | 缓存读 | 缓存写 | 批处理(入/出) | 其他计费 | 备注 |
|---|---|---|---|---|---|---|---|---|---|---|
| `abliterated-model` | — | reasoning | 150K | $3 | $3 | $0.3 | — | —/— | — | models.dev official list price |
| `abliterated-model-large` | — | reasoning | 1M | $5 | $5 | $0.5 | — | —/— | — | models.dev official list price |
