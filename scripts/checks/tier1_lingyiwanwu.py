"""lingyiwanwu official pricing check (tier 1). Best-effort fetch; on failure records that the data was manually reconciled (2026-08-28) so the check does not silently mis-update. Parser can be added once reliably fetchable."""
import sys
sys.path.insert(0, __file__.rsplit("checks", 1)[0])
from toolbox import js_fetch  # noqa: E402

TIER = 1
PROVIDER_ID = "lingyiwanwu"
