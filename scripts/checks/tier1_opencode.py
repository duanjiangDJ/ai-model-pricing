"""OpenCode Zen official pricing check (tier 1).

Source: https://opencode.ai/docs/zen/ — a SERVER-RENDERED HTML price table (plain
...
js_fetch and never parsed it, so `opencode` prices were maintained purely from the
...
def build_updates(parsed, provider, now=None):
    ...
    for key, pr in parsed.items():
        ...
        updates[ids[0]] = {
            "per_mtok": pm,
            "notes": ("OpenCode Zen official price list (opencode.ai/docs/zen/, USD per 1M "
...
def run(ctx):
    ...
    return {"changed": len(changed),
            "detail": f"opencode.ai/docs/zen/ table parsed ({len(parsed)} priced rows, "
