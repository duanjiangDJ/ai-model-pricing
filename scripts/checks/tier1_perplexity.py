"""perplexity official pricing check (tier 1). Vendor publishes USD-only list prices; a model present here is already single-currency USD (correct — no official CNY price page), so no CNY branch is added. Best-effort fetch; parser can be added once reliably fetchable."""
import sys
